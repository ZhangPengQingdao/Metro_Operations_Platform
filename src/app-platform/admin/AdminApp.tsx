...
import {Button,Input,Table,TableHeader,TableBody,TableRow,TableHead,TableCell,FilterBar} from '../../components/ui';
...
function RuntimeStatus({app}:{app:Installation}){
 const [serving,setServing]=useState<boolean|null>(null),[failed,setFailed]=useState(false);
 useEffect(()=>{const c=new AbortController();let pending=false;const load=async()=>{if(pending)return;pending=true;try{const result=await adminRequest<{serving:boolean}>(`/apps/${app.appId}/runtime-status`,{signal:c.signal});if(!c.signal.aborted){setServing(result.serving);setFailed(false);}}catch{if(!c.signal.aborted)setFailed(true);}finally{pending=false;}};void load();const timer=setInterval(()=>void load(),5000);return()=>{c.abort();clearInterval(timer);};},[app.appId,app.revision]);
 return <span className="admin-status">{!app.enabled?'已停用':failed?'状态待核对':serving===null?'检查中…':serving?'正在运行':'需要恢复'}</span>;
}
function AppsPage({onChange}:{onChange:()=>void}){
 ...
 <FilterBar showDateRange={false} searchValue={search} onSearchChange={setSearch} searchPlaceholder="搜索应用名称" rightAction={<div className="app-toolbar-actions"><Button size="sm" variant="secondary" disabled={!data?.installConfigured} onClick={()=>setPolicyOpen(true)}>发布者与审批</Button><Button size="sm" disabled={!data?.installConfigured} onClick={()=>{setInstall(true);setFailure('');setResult('');}}>安装应用</Button></div>}/>
 {result&&<p role="status">{result}</p>}{error&&<ErrorNotice error={error} reload={reload}/>}{!data&&!error&&<p role="status">正在读取应用…</p>}
 {data&&<Table variant="directory" wrapperClassName="app-management-table" emptyState={!data.applications.length?'尚未安装应用':undefined}><TableHeader><TableRow><TableHead>应用</TableHead><TableHead>版本</TableHead><TableHead>运行状态</TableHead><TableHead>数据授权</TableHead><TableHead>操作</TableHead></TableRow></TableHeader><TableBody>{data.applications.filter(app=>`${app.manifest.name} ${app.appId}`.toLowerCase().includes(search.toLowerCase())).map(app=><TableRow key={app.id}><TableCell><div className="app-name-cell"><strong>{app.manifest.name}</strong><span className="afc-muted">{app.appId}</span></div></TableCell><TableCell>{app.manifest.version}</TableCell><TableCell><RuntimeStatus app={app}/></TableCell><TableCell>{app.grants.filter(g=>g.status==='active').length} 项</TableCell><TableCell><div className="app-row-actions"><Button size="sm" variant="ghost" onClick={()=>setStatusApp(app.appId)}>状态</Button><Button size="sm" variant="ghost" onClick={()=>setGrantApp(app.appId)}>授权</Button><Button size="sm" variant="ghost" disabled={!data.runtimeConfigured} onClick={()=>{setSelected({app,action:'upgrade'});setFailure('');}}>更新</Button><Button size="sm" variant="ghost" disabled={!data.runtimeConfigured} onClick={()=>{setSelected({app,action:app.enabled?'disable':'enable'});setFailure('');}}>{app.enabled?'停用':'启用'}</Button></div></TableCell></TableRow>)}</TableBody></Table>}

 {policyOpen&&<PublisherPolicyDialog onClose={()=>setPolicyOpen(false)}/>}
...
export default function AdminApp(){
 ...
 const path=location.pathname;const content=path==='/admin'?<Overview/>:path==='/admin/apps'?<AppsPage onChange={reloadApps}/>:(path==='/admin/data'||path.startsWith('/admin/data/'))?<DataPage/>:path==='/admin/accounts/employees'?<EmployeeAccounts/>:path==='/admin/accounts'?<AccountsPage/>:path==='/admin/updates'?<SystemUpdates/>:path==='/admin/audit'?<AuditPanel/>:path==='/admin/developer'?<DeveloperPage/>:path.startsWith('/admin/app/')?<AdminApplicationOutlet/>:<section className="admin-card"><h1>页面不存在</h1><Link to="/admin">返回系统总览</Link></section>;
