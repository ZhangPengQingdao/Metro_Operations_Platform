...
import {Button,Input,QueryList,TableActions,TableActionButton,TableHeader,TableBody,TableRow,TableHead,TableCell} from '../../components/ui';
...
function FrontendModeDialog({app,trustedAvailable,onClose,onChanged}:{app:Installation;trustedAvailable:boolean;onClose:()=>void;onChanged:()=>void}){
 const [mode,setMode]=useState<'standard'|'trusted'>(app.frontendRunMode==='trusted'?'trusted':'standard');
 const [busy,setBusy]=useState(false),[error,setError]=useState('');
 async function save(){if(busy)return;setBusy(true);setError('');try{await adminRequest(`/apps/${app.appId}/frontend-mode`,{method:'PUT',body:{revision:app.revision,mode}});onChanged();onClose();}catch(e){setError(e instanceof Error?e.message:'运行模式未确认，请刷新后核对。');}finally{setBusy(false);}}
 return <AdminDialog title={`${app.manifest.name} · 应用设置`} onClose={()=>{if(!busy)onClose();}}><div className="admin-form"><fieldset className="app-frontend-mode"><legend>运行模式</legend><label><input type="radio" name="appFrontendMode" checked={mode==='standard'} disabled={busy} onChange={()=>setMode('standard')}/> 标准沙箱 <span className="afc-muted">强隔离，推荐普通应用</span></label><label><input type="radio" name="appFrontendMode" checked={mode==='trusted'} disabled={busy||!trustedAvailable||app.manifest.ui.mode!=='sandbox'} onChange={()=>setMode('trusted')}/> 可信应用 <span className="afc-muted">独立稳定 Origin、缓存和预取；仅供管理员认可的正式内部应用</span></label></fieldset><p className="afc-muted">切换后已打开的应用页面将重新加载。员工权限、业务权限和审计继续生效。</p>{error&&<p className="afc-error" role="alert">{error}</p>}<Button disabled={busy||mode===(app.frontendRunMode==='trusted'?'trusted':'standard')} onClick={()=>void save()}>{busy?'保存中…':'保存运行模式'}</Button></div></AdminDialog>;
}
function AppsPage({onChange}:{onChange:()=>void}){
 const {data,error,reload}=useRead<{applications:Installation[];runtimeConfigured:boolean;installConfigured:boolean;trustedAvailable:boolean}>('/apps');
 const [search,setSearch]=useState('');
 const filteredApps=data?.applications.filter(app=>`${app.manifest.name} ${app.appId}`.toLowerCase().includes(search.toLowerCase()))??[];
 const [selected,setSelected]=useState<{app:Installation;action:'enable'|'disable'|'upgrade'}|null>(null),[grantApp,setGrantApp]=useState<string|null>(null),[install,setInstall]=useState(false),[policyOpen,setPolicyOpen]=useState(false),[statusApp,setStatusApp]=useState<string|null>(null),[settingsApp,setSettingsApp]=useState<Installation|null>(null),[result,setResult]=useState(''),[busy,setBusy]=useState(false),[failure,setFailure]=useState('');
 ...
 {data&&<QueryList query={{showDateRange:false,searchValue:search,onSearchChange:setSearch,searchPlaceholder:'搜索应用名称'}} actions={<div className="app-toolbar-actions"><Button size="sm" variant="secondary" onClick={()=>setPolicyOpen(true)}>审核与发布</Button><Button size="sm" leadingIcon={<Plus size={18}/>} disabled={!data?.installConfigured} onClick={()=>{setInstall(true);setFailure('');setResult('');}}>安装应用</Button></div>} emptyState={!filteredApps.length?(search?'没有匹配的应用':'尚未安装应用'):undefined}><TableHeader><TableRow><TableHead>应用</TableHead><TableHead>版本</TableHead><TableHead>运行状态</TableHead><TableHead>平台能力</TableHead><TableHead>操作</TableHead></TableRow></TableHeader><TableBody>{filteredApps.map(app=><TableRow key={app.id}><TableCell><div className="app-name-cell"><strong>{app.manifest.name}</strong><span className="afc-muted">{app.appId}</span></div></TableCell><TableCell>{app.manifest.version}</TableCell><TableCell><RuntimeStatus app={app}/></TableCell><TableCell>{app.grants.filter(g=>g.status==='active'&&g.permissionCode.startsWith('platform.')).length} 项</TableCell><TableCell><TableActions><TableActionButton icon={<Pulse size={18}/>} onClick={()=>setStatusApp(app.appId)}>状态</TableActionButton><TableActionButton icon={<GearSix size={18}/>} onClick={()=>setSettingsApp(app)}>设置</TableActionButton><TableActionButton icon={<ShieldCheck size={18}/>} onClick={()=>setGrantApp(app.appId)}>授权</TableActionButton><TableActionButton icon={<UploadSimple size={18}/>} disabled={!data.runtimeConfigured} onClick={()=>{setSelected({app,action:'upgrade'});setFailure('');}}>更新</TableActionButton><TableActionButton icon={app.enabled?<Pause size={18}/>:<Play size={18}/>} disabled={!data.runtimeConfigured} onClick={()=>{setSelected({app,action:app.enabled?'disable':'enable'});setFailure('');}}>{app.enabled?'停用':'启用'}</TableActionButton></TableActions></TableCell></TableRow>)}</TableBody></QueryList>}
 ...
 {settingsApp&&<FrontendModeDialog app={settingsApp} trustedAvailable={!!data?.trustedAvailable} onClose={()=>setSettingsApp(null)} onChanged={()=>{reload();onChange();setResult('运行模式已更新。');}}/>}
