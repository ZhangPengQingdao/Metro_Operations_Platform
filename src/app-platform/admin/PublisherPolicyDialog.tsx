...
export function PublisherPolicyDialog({onClose}:{onClose:()=>void}){
 ...
 return <AdminDialog className="afc-profile-dialog afc-sidebar-dialog" title="发布者与审批" onClose={()=>{if(!busy)onClose();}}>
  <div className="afc-profile-layout">
   <nav className="afc-profile-nav" aria-label="发布者与审批">
    {SECTIONS.map(item=><Button key={item.id} variant="ghost" shape="rounded" disabled={busy} aria-current={section===item.id?'page':undefined} leadingIcon={item.icon} onClick={()=>setSection(item.id)}>{item.label}</Button>)}
   </nav>
