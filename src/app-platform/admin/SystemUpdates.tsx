...
const phases:Record<string,string>={queued:'等待处理',downloading:'下载中',verified:'校验通过',downloaded:'已下载',preflight:'升级预检',maintenance:'进入维护',backing_up:'正在备份',migrating:'数据库迁移',switching:'切换版本',health_check:'健康检查',completed:'更新完成',failed:'操作失败',recovery_required:'需要服务器核对'};
export function SystemUpdates(){
 ...
 const task=status?.task;const running=!!task&&!['downloaded','completed','failed','recovery_required'].includes(task.phase);
