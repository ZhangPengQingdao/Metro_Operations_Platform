...
export function AiSettings({onSaved,onBusy}:{onSaved?:()=>void;onBusy?:(busy:boolean)=>void}){
 ...
 async function run(action:'save'|'models'|'test'){
  if(busy)return;setBusy(action);onBusy?.(true);setError('');setStatus('');
  ...
  try{
   if(action==='save'){const next=await adminRequest<Config>('/ai/save',{method:'POST',body});setConfig(next);setEndpoint(next.external.endpoint);setKey('');setStatus('已保存');onSaved?.();}
  ...
  }catch(e){setError(e instanceof Error?e.message:'操作失败');}finally{setBusy('');onBusy?.(false);}
 }
