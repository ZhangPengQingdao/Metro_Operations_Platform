...
const messages:Record<string,string>={ADMIN_AUTH_REQUIRED:'管理员登录已失效，请重新登录。',ADMIN_LOGIN_FAILED:'用户名或密码不正确。',ADMIN_DATABASE_UNAVAILABLE:'管理员数据库尚未配置。',ADMIN_SERVICE_UNAVAILABLE:'管理服务不可用，请检查数据库和迁移配置。',ADMIN_ORIGIN_DENIED:'当前访问地址与平台配置不一致。',ADMIN_RATE_LIMIT:'尝试次数过多，请稍后再试。',APP_RUNTIME_NOT_CONFIGURED:'应用运行宿主尚未配置。',APP_INSTALL_NOT_CONFIGURED:'安装服务尚未配置。',ADMIN_INVALID_INPUT:'请检查填写内容。',STALE_REVISION:'数据已变化，请刷新后重新核对。'};
