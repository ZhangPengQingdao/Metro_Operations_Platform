...
import {Button,Input,Select,DataList,TableActionButton} from '../../components/ui';
...
export function EmployeeAppAccess({appId}:{appId:string}){
 ...
 return <section><h3>员工使用范围</h3>{error&&<p className="afc-error" role="alert">{error}</p>}<DataList emptyState={grants?.length===0?'尚未分配员工':undefined}><thead><tr><th>员工</th><th>状态</th><th>操作</th></tr></thead><tbody>{grants?.map(g=><tr key={g.personId}><td>{g.name} · {g.employeeNo}</td><td>{g.enabled?'允许使用':'已撤销'}</td><td><TableActionButton icon={g.enabled?<UserMinus size={18}/>:<UserPlus size={18}/>} disabled={busy} onClick={()=>void save(g.personId,!g.enabled)}>{g.enabled?'撤销使用':'允许使用'}</TableActionButton></td></tr>)}</tbody></DataList><form className="admin-form" onSubmit={e=>{e.preventDefault();void save(personId,true);}}><label>查找员工<Input value={search} onChange={e=>setSearch(e.target.value)} placeholder="姓名或工号" disabled={busy}/></label><label>员工<Select required value={personId} onChange={e=>setPersonId(e.target.value)} disabled={busy}><option value="">请选择</option>{people.filter(p=>!grants?.some(g=>g.personId===p.id&&g.enabled)).map(p=><option key={p.id} value={p.id}>{p.name} · {p.employeeNo}</option>)}</Select></label><Button type="submit" disabled={busy||!personId||!grants}>允许使用</Button></form></section>;
}
