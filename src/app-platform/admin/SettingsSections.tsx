...
export function SettingsSections(){
 const [section,setSection]=useState<'ai'|'updates'|'audit'>('ai');
 const active=sections.find(item=>item.id===section)??sections[0];
 return <div className="afc-profile-layout">
  <nav className="afc-profile-nav" aria-label="系统设置">
   {sections.map(item=><Button key={item.id} variant="ghost" shape="rounded" aria-current={section===item.id?'page':undefined} leadingIcon={<PlatformIcon name={item.icon} size={18}/>} onClick={()=>setSection(item.id)}>{item.label}</Button>)}
  </nav>
