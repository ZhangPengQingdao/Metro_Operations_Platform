...
export function InstallPackageReview({target,onDone,onBusy}:{target?:{appId:string;revision:number};onDone:(message:string)=>void;onBusy:(busy:boolean)=>void}){
 ...
 return <div className="admin-form"><label>签名安装包<Input type="file" accept="application/json,.json" disabled={busy} onChange={e=>{setFile(e.target.files?.[0]??null);setPreview(null);setBundle(null);setError('');}}/></label><Button variant="secondary" disabled={busy||!file} onClick={()=>void inspect()}>校验并预览</Button>{error&&<p className="afc-error" role="alert">{error}</p>}{preview&&<><dl className="afc-details"><dt>应用</dt><dd>{preview.manifest.name} · {preview.manifest.id}</dd><dt>版本</dt><dd>{preview.installed?`${preview.installed.version} → `:''}{preview.manifest.version}</dd><dt>发布者</dt><dd>{preview.manifest.publisherId} · {preview.keyId}</dd><dt>签名</dt><dd>已通过可信公钥校验</dd><dt>申请权限</dt><dd>{preview.manifest.permissions.requested.join('、')||'无'}</dd><dt>自定义权限</dt><dd>{preview.manifest.permissions.defined.map(p=>p.code).join('、')||'无'}</dd><dt>运行方式</dt><dd>{preview.manifest.ui.mode} / {preview.manifest.backend.mode}</dd><dt>存储需求</dt><dd>{preview.manifest.storage.mode}</dd><dt>新增迁移</dt><dd>{preview.changes.addedMigrations.join('、')||'无'}</dd><dt>权限变化</dt><dd>新增：{preview.changes.addedPermissions.join('、')||'无'}；移除：{preview.changes.removedPermissions.join('、')||'无'}</dd><dt>扩展声明</dt><dd>接口 {preview.manifest.api.length} · 工具 {preview.manifest.tools.length} · 任务 {preview.manifest.jobs.length} · 事件 {preview.manifest.events.publish.length+preview.manifest.events.subscribe.length}</dd><dt>外联域名</dt><dd>{[...preview.manifest.network.frontendOrigins,...preview.manifest.network.backendOrigins].join('、')||'无'}</dd><dt>产物大小</dt><dd>{preview.manifest.artifacts.reduce((n,a)=>n+a.bytes,0).toLocaleString()} 字节</dd><dt>版本审批</dt><dd>{preview.approved?'已批准':'待批准'}</dd></dl>{(!preview.supported||!preview.compatible)&&<p className="afc-error" role="alert">此安装包包含尚未接通的能力或不兼容的存储变更，无法批准。</p>}<p className="afc-muted">版本审批不授予员工使用权或数据权限。</p><Button disabled={busy||!preview.supported||!preview.compatible} onClick={()=>void install()}>{busy?'正在处理…':target?'批准此版本并更新':'批准此版本并安装'}</Button></>}</div>;
}
