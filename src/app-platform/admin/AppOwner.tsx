...
export function AppOwner({appId,onMode}:{appId:string;onMode?:(mode:string)=>void}){
 ...
 return <section><h3>应用负责人</h3>{error&&<p role="alert" className="afc-error">{error}</p>}<form className="admin-form" onSubmit={async e=>{e.preventDefault();if(!state||busy)return;setBusy(true);setError('');try{setState(await adminRequest(path,{method:'PUT',body:{personId:person||null,revision:state.revision}}));}catch(e){setError((e as Error).message);}finally{setBusy(false);}}}>
 <label>查找人员<Input value={search} placeholder="姓名或工号" onChange={e=>setSearch(e.target.value)}/></label><label>负责人<Select value={person} disabled={busy} onChange={e=>setPerson(e.target.value)}><option value="">未指定</option>{state?.owner&&!people.some(p=>p.id===state.owner!.id)&&<option value={state.owner.id}>{state.owner.name}</option>}{people.map(p=><option key={p.id} value={p.id}>{p.name} · {p.employeeNo}</option>)}</Select></label><Button type="submit" disabled={!state||busy||person===(state.owner?.id??'')}>保存负责人</Button></form></section>;
}
