...
export function ApplicationStatusDialog({appId,onClose,onChange}:{appId:string;onClose:()=>void;onChange:()=>void}){
 const [runtime,setRuntime]=useState<RuntimeStatus|null>(null),[install,setInstall]=useState<InstallStatus|null>(null),[versions,setVersions]=useState<VersionStatus[]>([]),[error,setError]=useState(''),[busy,setBusy]=useState(false),[serial,setSerial]=useState(0);
 ...
 return <AdminDialog title="应用状态" onClose={()=>{if(!busy)onClose();}}>{error&&<p role="alert" className="afc-error">{error}</p>}{runtime?<><dl className="afc-details"><dt>当前版本</dt><dd>{runtime.installation.manifest.version}</dd><dt>运行状态</dt><dd>{runtime.serving?'正在运行':runtime.installation.enabled?'需要恢复运行状态':'已停用'}</dd><dt>数据授权</dt><dd>{runtime.installation.grants.filter(g=>g.status==='active').length} 项已批准</dd><dt>安装记录</dt><dd>{install?(states[install.state]??install.state):'未找到'}</dd>{versions[0]&&<><dt>最近更新</dt><dd>{versions[0].version} · {states[versions[0].state]??versions[0].state}</dd></>}</dl>{blocked&&<section><h3>未完成的应用调用</h3><p className="afc-error" role="alert">原调用缺少结束凭据，恢复和启用已阻断。不会自动重试或清除记录。</p><DataList pinActions={false}><thead><tr><th>开始时间</th><th>调用</th><th>核对编号</th></tr></thead><tbody>{runtime.pendingWork!.records.map(w=><tr key={w.id}><td>{new Date(w.startedAt).toLocaleString('zh-CN')}</td><td>{w.context?.apiId??'历史调用（未记录接口）'}</td><td>{w.id}</td></tr>)}</tbody></DataList>{runtime.pendingWork!.hasMore&&<p>仅显示前 100 条，仍有其他未完成调用。</p>}</section>}<div className="admin-actions">{!runtime.installation.enabled&&runtime.installation.manifest.backend.mode==='isolated'&&!pendingVersion&&!pendingInstall&&<Button disabled={busy||blocked} onClick={()=>void recover('prepare-credential',{revision:runtime.installation.revision})}>重新准备运行身份</Button>}{pendingVersion?<Button disabled={busy||blocked} onClick={()=>void recover('version-recover',{requestId:pendingVersion.requestId})}>恢复为停用</Button>:pendingInstall?<Button disabled={busy||blocked} onClick={()=>void recover('install-recover',{revision:install!.revision})}>恢复为停用</Button>:(!runtime.serving&&(runtime.installation.enabled||runtime.installation.lifecycle?.status==='failed'||runtime.installation.lifecycle?.status==='running'))&&<Button disabled={busy||blocked} onClick={()=>void recover('recover',{revision:runtime.installation.revision})}>恢复为停用</Button>}</div>{runtime.installation.manifest.storage.mode==='managed'&&<><AppMigrationStatus appId={appId} onChange={()=>{onChange();setSerial(v=>v+1);}}/><AppMigrationStatus kind="writes" appId={appId} onChange={()=>{onChange();setSerial(v=>v+1);}}/></>}</>:!error&&<p role="status">正在读取状态…</p>}<div className="admin-actions"><Button variant="secondary" disabled={busy} onClick={()=>setSerial(v=>v+1)}>重新核对</Button></div></AdminDialog>;
}
