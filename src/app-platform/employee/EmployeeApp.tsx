...
export default function EmployeeApp(){
 ...
 const visibleApps=[...apps,...owned.filter(o=>!apps.some(a=>a.appId===o.appId)).map(o=>({...o,description:'应用权限管理',version:'',navigation:[],routes:[]}))];
 const applications:AdminApplication[]=visibleApps.map(app=>({id:app.appId,name:app.name,navigation:[...app.navigation.filter(item=>!menus[app.appId]||menus[app.appId].includes(item.id)).flatMap(item=>{const route=app.routes.find(r=>r.id===item.routeId);return route?[{id:item.id,label:item.label,path:`/employee/app/${app.appId}${route.path==='/'?'':route.path}`}]:[]}),...(owned.some(o=>o.appId===app.appId)?[{id:'platform-management',label:'应用管理',path:`/employee/app/${app.appId}/~management`}]:[])]}));
