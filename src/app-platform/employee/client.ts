export class EmployeeRequestError extends Error{constructor(readonly status:number,readonly code:string){super(({EMPLOYEE_LOGIN_FAILED:'用户名或密码不正确。',EMPLOYEE_AUTH_REQUIRED:'登录已失效，请重新登录。',EMPLOYEE_RATE_LIMIT:'尝试次数过多，请稍后再试。',EMPLOYEE_APP_ACCESS_DENIED:'此应用的使用权限已撤销。',APP_RESOURCE_ORIGIN_NOT_CONFIGURED:'应用资源域尚未配置。',ACCESS_DENIED:'当前无权执行此操作。'} as Record<string,string>)[code]??`操作未确认（${code}），请核对后再操作。`);}}
