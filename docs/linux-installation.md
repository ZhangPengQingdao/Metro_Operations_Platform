...
安装升级前须停用业务应用并排空工作；未确认的应用存储写入、运行工作、迁移或租约会阻断。安装确认提交期间弹窗保留处理提示，后台受理后展示任务状态。升级期间网页可能断连，恢复服务后按任务编号核对；结果未知时不会自动重放，不要因超时重复提交。更新器在宿主运行，不随 API 容器退出。
...
新发行清单要求宿主更新器协议 v4。正常更新按签名镜像摘要拉取缺失层；全量归档保留为显式备用传输。完成首次旧版过渡后，平台更新可保存应用运行快照、统一暂停并在平台健康后恢复此前运行的应用，具体兼容与恢复边界见 [分层更新说明](layered-updates.md)。
