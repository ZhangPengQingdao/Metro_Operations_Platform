...
const style=document.createElement('style');style.nonce=script?.nonce??'';style.textContent=':root{font-family:system-ui,sans-serif;color:#27272a;background:#fafafa}body{margin:0;padding:24px}form{background:#fff;border:1px solid #e4e4e7;border-radius:16px;padding:16px;margin:16px 0}h1{font-size:24px}h2{font-size:18px}label{display:inline-block;margin:6px 0}input{border:1px solid #d4d4d8;border-radius:8px;padding:8px;font:inherit}button{border:1px solid #d4d4d8;border-radius:999px;padding:8px 16px;margin:8px 8px 0 0;background:white;font:inherit;cursor:pointer}button:disabled{opacity:.5;cursor:default}table{border-collapse:collapse;font-size:14px;width:100%;margin-top:16px}td,th{padding:10px;border-bottom:1px solid #e4e4e7;text-align:left;overflow-wrap:anywhere}#app{max-width:1200px;margin:auto}';document.head.append(style);
const root=document.getElementById('app');
const element=(tag,text)=>{const node=document.createElement(tag);if(text)node.textContent=text;return node;};
const lastRequest=element('p');root.append(lastRequest);
...
const layout=element('div'),sidebar=element('nav'),content=element('main');layout.className='layout';sidebar.setAttribute('aria-label','物料应用导航');layout.append(sidebar,content);root.append(layout);
...
function renderNavigation(){sidebar.replaceChildren();for(const name of sections.keys()){if(!allowed(name))continue;const button=element('button',name);button.type='button';button.onclick=()=>void navigate(name);sidebar.append(button);}}
...
const toolbar=element('div'),search=element('input');search.placeholder='物料名称';toolbar.append(search);
const output=element('div');let kind='materials',cursor=null;
async function load(next=false){const result=await call('list',{kind,...(next&&cursor?{afterId:cursor}:{}),...(kind==='materials'&&search.value?{search:search.value}:{})});if(!result)return;cursor=result.nextCursor;output.replaceChildren();const table=element('table');const columns=kind==='materials'?['id','name','sku','unit','quantity']:['id','material_id','kind','delta','operator_id','created_at','reverses_id','remark'];const tr=element('tr');const labels={id:'编号',name:'名称',sku:'SKU',unit:'单位',quantity:'库存',material_id:'物料编号',kind:'类型',delta:'变动数量',operator_id:'操作人编号',created_at:'时间',reverses_id:'原流水编号',remark:'备注'};for(const c of columns)tr.append(element('th',labels[c]));table.append(tr);for(const row of result.rows){const tr=element('tr');for(const c of columns)tr.append(element('td',String(row[c]??'')));table.append(tr);}output.append(table);nextButton.disabled=!cursor;}
const refresh=element('button','查询');refresh.onclick=()=>void load();toolbar.append(refresh);
const nextButton=element('button','下一页');nextButton.disabled=true;nextButton.onclick=()=>void load(true);toolbar.append(nextButton);
window.addEventListener('pagehide',()=>sandbox.close(),{once:true});

const settings=sections.get('设置'),memberSearch=element('input'),memberList=element('div');memberSearch.placeholder='姓名或工号';const findMembers=element('button','查询成员');settings.append(element('h2','工班物资管理员'),memberSearch,findMembers,memberList);findMembers.onclick=()=>void loadMembers();
