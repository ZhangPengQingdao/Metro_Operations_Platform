...
- 按日期范围导出 `.xlsx`，单次最多 5000 条，超限明确要求分段导出。
- 车站目录可用时提供名称建议；设备类型、编号允许手工录入。处理人必须在所选工班的在职人员目录中。
...
应用服务需获 `platform.app_data.read/write`、`platform.people.read`、`platform.locations.read`；员工还需获应用使用授权及对应业务权限。平台管理员身份不代替员工业务身份。未知写入结果不自动重试，界面保留请求编号并阻止后续写入。
