...
export function buildSandboxDocument(input: SandboxDocumentInput): SandboxDocument {
  ...
  if(input.trusted&&!/^[a-z][a-z0-9]*(?:-[a-z0-9]+)*$/.test(input.trusted.appId))throw new Error('SANDBOX_INVALID_APP_ID');
  const style = input.style ? verified(input.style, 256 * 1024) : '';
  const nonce = randomBytes(24).toString('base64');
  const csp = `default-src 'none'; script-src 'nonce-${nonce}'${input.trusted?" 'self'":''}; style-src 'nonce-${nonce}'; img-src data:; font-src data:; connect-src 'none'; frame-src 'none'; worker-src 'none'; object-src 'none'; base-uri 'none'; form-action 'none'`;
  ...
  const scriptAttributes=`data-app-route="${encodeURIComponent(input.route??'/')}" data-platform-origin="${origin.origin.replace(/&/g,'&amp;').replace(/"/g,'&quot;')}"`;
  const entry=input.trusted?`<script ${scriptAttributes} src="/assets/${input.trusted.appId}/${input.script.sha256}/ui.js"></script>`:`<script nonce="${nonce}" ${scriptAttributes}>${script.replace(/<\/script/gi,'<\\/script')}</script>`;
