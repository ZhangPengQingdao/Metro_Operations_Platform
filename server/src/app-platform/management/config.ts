...
export interface AppManagementConfig {version:1;artifactRoot:string;runtimeRoot:string;uploadRoot:string;publisherPolicyFile:string;approvedManifestDigests:string[];docker?:{socketPath:string;runtimeImage:string;approval:{imageId:string;config:Record<string,unknown>}}}
...
export function isAppRuntimeSupported(manifest:AppManifest,config:AppManagementConfig){
 ...
 return ['none','sandbox'].includes(manifest.ui.mode)
  ...
  && manifest.storage.mode==='none' && !manifest.events.publish.length && !manifest.events.subscribe.length
