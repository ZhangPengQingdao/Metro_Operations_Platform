...
export interface ApprovalDocument{revision:number;policy:PublisherPolicy;approvedManifestDigests:string[]}
...
async function read(db:QueryableClient):Promise<ApprovalDocument|null>{
 const result=await db.query('SELECT revision,document FROM platform_app_approval_history ORDER BY revision DESC LIMIT 1') as {rows:{revision:number;document:{policy:unknown;approvedManifestDigests:unknown}}[]};
 ...
 return {revision:row.revision,policy,approvedManifestDigests:digests.parse(row.document.approvedManifestDigests)};
}
export class AppApprovalStore{
 ...
 async save(context:PlatformManagementContext,revision:number,change:{keys?:PublisherPolicy['keys'];approval?:{digest:string;approved:boolean}}){
  ...
  return this.use(db=>runDatabaseTransaction(db,async()=>{
   ...
   const document={policy,approvedManifestDigests:digests.parse([...approved])};
