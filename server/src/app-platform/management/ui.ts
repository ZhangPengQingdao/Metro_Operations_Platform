...
export async function readInstalledAdminUi(options:{context:PlatformManagementContext;host:Pick<AppLifecycleHost,'status'>;appId:string;path:string;origin:string;readArtifact(manifest:AppManifest,id:string,bytes:number):Promise<Uint8Array>}){
 ...
 if(origin.protocol!=='http:'||!['127.0.0.1','[::1]'].includes(origin.hostname))throw new AppManagementError('APP_RESOURCE_ORIGIN_NOT_CONFIGURED');
 ...
 return {appId:manifest.id,name:manifest.name,instanceKey:`${installation.id}:${installation.revision}:${options.path}`,resource:{mode:'local-demo' as const,html:document.html,platformOrigin:origin.origin}};
}
