...
export async function readInstalledAdminUi(options:{context:PlatformManagementContext;host:Pick<AppLifecycleHost,'status'>;appId:string;path:string;origin:string;publish?:(document:SandboxDocument,authorize:()=>Promise<void>,trustedAppId?:string)=>{mode:'isolated-origin';url:string;platformOrigin:string;frontendRunMode?:'trusted'};readArtifact(manifest:AppManifest,id:string,bytes:number):Promise<Uint8Array>}){
 ...
 const document=buildSandboxDocument({platformOrigin:origin.origin,route:options.path,allowDownloads:manifest.ui.downloads===true,script:{text:Buffer.from(bytes).toString('utf8'),bytes:artifact.bytes,sha256:artifact.sha256},...(trusted?{trusted:{appId:options.appId}}:{})});
 ...
 return {appId:manifest.id,name:manifest.name,...(manifest.ui.downloads?{downloads:true as const}:{}),requiresEmployee:manifest.api.some(api=>api.businessEntry||api.businessPermission),instanceKey:`${installation.id}:${installation.revision}:${options.path}`,resource:options.publish?options.publish(document,async()=>{await assertInstallAdmin(options.context);const current=await options.host.status(options.context);if(!current.serving||current.installation.revision!==installation.revision)throw new AppManagementError('APP_NOT_SERVING');},trusted?options.appId:undefined):{mode:'local-demo' as const,html:document.html,platformOrigin:origin.origin}};
}
