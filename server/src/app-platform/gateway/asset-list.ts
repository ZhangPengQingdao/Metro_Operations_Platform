...
export function createAssetListReader(client:QueryableClient){
 return async(value:unknown)=>{
  ...
  const result=await client.query(`SELECT a.id, a.display_name AS "displayName", a.asset_code AS "assetCode",
    a.location_id AS "locationId", a.type_id AS "typeId", t.name AS "typeName", a.lifecycle_state AS "lifecycleState",
    l.organization_unit_id AS "organizationUnitId"
    FROM platform_assets a
    JOIN platform_locations l ON l.id = a.location_id
    JOIN platform_asset_types t ON t.id = a.type_id
    WHERE ($1::uuid IS NULL OR a.id > $1::uuid)
      ...
      AND ($4::text IS NULL OR strpos(lower(coalesce(a.asset_code, '') || ' ' || a.display_name || ' ' || t.name), lower($4)) > 0)
      AND ($5::uuid IS NULL OR a.location_id = $5::uuid)
      AND ($6::uuid IS NULL OR a.type_id = $6::uuid)
      AND ($7::text IS NULL OR t.name = $7::text)
    ORDER BY a.id LIMIT $8`,
   [p.afterId ?? null, p.organizationUnitId ?? null, p.lifecycleState ?? null, p.search ?? null, p.locationId ?? null, p.typeId ?? null, p.typeName ?? null, p.pageSize + 1]);
  return (result as {rows:unknown[]}).rows as {id:string;displayName:string;assetCode:string|null;locationId:string;typeId:string;typeName:string;lifecycleState:string;organizationUnitId:string|null}[];
 };
}
