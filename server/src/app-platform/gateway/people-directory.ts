...
const params=z.object({organizationUnitId:z.string().uuid(),personId:z.string().uuid().optional(),search:z.string().max(100).optional()}).strict();
...
export function createPeopleDirectoryOperation(people:Pick<PeopleDirectoryRepository,'searchPeople'|'getPersonProfile'>):AppGatewayOperation{
 return {name:'platform.people.members',permissionCode:'platform.people.read',mode:'read',
  ...
  async execute(_context,value){
   ...
   const profiles=p.personId?[await people.getPersonProfile(p.personId)]:await people.searchPeople({organizationUnitId:p.organizationUnitId,employmentStatus:'active',query:p.search,limit:50});
