...
export async function createAppProject(destination: string, appId: string, mode: string, publisherId: string) {
  ...
  const template = new URL(mode==='backend'?'../../../../examples/backend-sdk/':'../../../../examples/app-sdk/', import.meta.url);
  ...
  for (const path of ['app.mjs', 'test.mjs', 'build.mjs', 'package.json', '.gitignore', ...(mode==='backend'?['entry.mjs','config.mjs']:[`${mode}/entry.mjs`])]) {
    files.set(path, await readFile(new URL(path, template), 'utf8'));
  }
