...
export class AppInstaller {
 ...
 async install(context:PlatformManagementContext,input:{requestId:string;directory:string;signatureFile:string}){
  ...
  const record=await runAtomicOperation([this.options.registry,this.options.journal],async()=>{
   ...
   const installation=await this.options.registry.register(context,prepared.manifest,'Signed package first installation');
