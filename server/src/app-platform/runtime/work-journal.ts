...
export class AppRuntimeWorkJournal {
 ...
 async track<T>(installationId:string,assertOwner:()=>Promise<void>,invoke:()=>Promise<T>,drain:()=>Promise<void>):Promise<T>{
  this.validate(installationId);const id=randomUUID();
  await this.query('INSERT INTO public.platform_app_runtime_work(id,installation_id) VALUES($1,$2)',[id,installationId]);
