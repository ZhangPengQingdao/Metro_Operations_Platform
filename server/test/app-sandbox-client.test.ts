...
function fixture(timeoutMs=100){
 const parent={},child={};let listener:Parameters<AppSandboxPort['listen']>[0]=()=>{};let calls=0;
 ...
 const client=createAppSandboxClient({appId:'sample',platformOrigin:origin,timeoutMs,port:{parent,listen:fn=>{listener=fn;return()=>{listener=()=>{};}},send:(data,target)=>{assert.equal(target,origin);void broker.receive({source:child,origin:'null',data});}}});
 const init=(source=parent,from=origin)=>listener({source,origin:from,data:{version:'1.0',type:'init',appId:'sample',session}});
 return {client,broker,init,calls:()=>calls,receive:(data:unknown)=>listener({source:parent,origin,data})};
}
