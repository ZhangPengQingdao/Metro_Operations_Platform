...
function fixture() {
 const now=new Date().toISOString();let allowed=true, calls=0, resolves=0;
 ...
 manifestVersion:'1.0',id:'demo',version:'1.0.0',name:'Demo',description:'Test app',publisherId:'example',compatibility:{platform:{minInclusive:'0.0.1-alpha.1',maxExclusive:'2.0.0'},capabilities:[],applications:[]},permissions:{requested:['app.demo.use'],defined:[]},ui:{mode:'none'},backend:{mode:'isolated',runtime:'node',entryArtifactId:'back',limits:{memoryMiB:128,cpuMillis:500,timeoutSeconds:10}},storage:{mode:'none'},routes:[],api:[{id:'read',method:'GET',path:'/read',handler:'read',permission:'app.demo.use'}],navigation:[],events:{publish:[],subscribe:[]},tools:[],jobs:[],network:{frontendOrigins:[],backendOrigins:[]},resources:[],artifacts:[{id:'back',kind:'backend',path:'back.js',sha256:'a'.repeat(64),bytes:1}]}};
 const actor={actorType:'person',trustedIdentity:{source:'wecom',externalUserId:'user'},execution:{type:'platform'},request:{requestId:'r',traceId:'t',startedAt:now},authorize:async()=>({allowed})} as unknown as PlatformActorContext;
 let execute=async()=>({ok:true});
 const api=createHostedAppApi({installation,getInstallation:async()=>installation,contextResolver:{resolve:async()=>{resolves++;return {...actor,execution:{type:'application',appId:'demo'}} as PlatformActorContext;}},authorize:async()=>allowed,
 transport:{invoke:async()=>{calls++;return execute();},drain:async()=>{}}});
 return {api,actor,request:{apiId:'read',method:'GET',path:'/read',payload:{}},get calls(){return calls;},get resolves(){return resolves;},disable:()=>{installation={...installation,enabled:false};},deny:()=>{allowed=false;},execute:(fn:typeof execute)=>{execute=fn;}};
}
